...
MyViewer::MyViewer(QWidget *parent) :
  ...
  show_control_points(true), show_solid(true), show_wireframe(false), show_colored_patches(false),
...
double MyViewer::voronoiWeight(MyViewer::PolyMesh::HalfedgeHandle in_he) {
...
void MyViewer::updateMeanCurvature(bool update_min_max) {
  std::map<PolyMesh::FaceHandle, double> face_area;
  std::map<PolyMesh::VertexHandle, double> vertex_area;
...
void MyViewer::fairMesh() {
  ...
  OpenMesh::Smoother::JacobiLaplaceSmootherT<PolyMesh> smoother(mesh);
  smoother.initialize(OpenMesh::Smoother::SmootherT<PolyMesh>::Normal, // or: Tangential_and_Normal
                      OpenMesh::Smoother::SmootherT<PolyMesh>::C1);
...
bool MyViewer::openBezier(const std::string &filename, bool update_view) {
  ...
  try {
    ...
    f >> n >> m;
    opened_bezier = new BezierSurface(n++, m++);
    std::vector<Vec> cps;
    cps.resize(n * m);
    for (size_t i = 0, index = 0; i < n; ++i) {
        for (size_t j = 0; j < m; ++j, ++index) {
            f >> cps[index][0] >> cps[index][1] >> cps[index][2];
            opened_bezier->addControlPoint(cps[index]);
        }
    }
...
bool MyViewer::saveBezier(const std::string &filename) {
  ...
  try {
    ...
    f.exceptions(std::ios::failbit | std::ios::badbit);
    f << opened_bezier->getDegree()[0] << ' ' << opened_bezier->getDegree()[1] << std::endl;
    for (const auto &p : opened_bezier->getCps())
...
void MyViewer::drawControlNet() const {
  ...
  auto degree = opened_bezier->getDegree();
  auto cps = opened_bezier->getCps();
  size_t m = degree[1] + 1;
...
void MyViewer::postSelection(const QPoint &p) {
  ...
  selected_vertex = sel;
  if (model_type == ModelType::MESH)
    axes.position = Vec(mesh.point(PolyMesh::VertexHandle(sel)).data());
  if (model_type == ModelType::BEZIER_SURFACE)
    axes.position = opened_bezier->getCps()[sel];
...
void MyViewer::mouseMoveEvent(QMouseEvent *e) {
  ...
  if (model_type == ModelType::MESH)
    mesh.set_point(PolyMesh::VertexHandle(selected_vertex),
                   Vector(static_cast<double *>(axes.position)));
  if (model_type == ModelType::BEZIER_SURFACE)
      opened_bezier->getCps()[selected_vertex] = axes.position;
  updateMesh();
  update();
}
